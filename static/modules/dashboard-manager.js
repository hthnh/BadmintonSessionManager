...
// Description: Updated to align with the new fixed APIs and SocketIO events.
...
/**
 * A wrapper for the fetch API to make calls to the backend.
...
async function apiCall(url, method = 'GET', body = null) {
    ...
    try {
        ...
        alert(`An error occurred: ${error.message}`); // Simple feedback for now
        return null;
    }
}
    ...
    activePlayers.forEach(player => {
        ...
        div.innerHTML = `
            ...
            <span class="player-elo">
                - Matches: <strong style="color: var(--color-primary);">${player.session_matches_played || 0}</strong>
            </span>`;
...
function renderScoreboardManager(allBoards, allCourts) {
    ...
    if (courts.length === 0) {
        ...
        courts.forEach(court => {
            ...
            html += `<select class="scoreboard-assign-select" data-court-id="${court.id}">${options}</select></div>`;
        });
    }
...
async function checkSessionStatus() {
    ...
    if (session) {
        ...
        statusText.innerHTML = `<span style="color: #198754; font-weight: 700;">ACTIVE</span> (Started: ${startTime})`;
...
async function handleRemoteControl(e) {
    ...
    } else if (courtId && action) {
        // We no longer need to check swap state here, backend handles it.
...
async function handleAssignAndBeginClick(event) {
    ...
    if (matchId) await openAssignCourtModal(matchId);
}
...
async function handleFinishMatchClick(event) {
    ...
    // If score is valid, ask for confirmation directly
    ...
    } else { // Otherwise, open the modal for manual input
...
async function handleAssignScoreboard(e) {
    ...
    if (result) {
        alert(result.message);
        await openScoreboardManager(); // Refresh the manager view
    }
}
...
async function handleConfirmAssignAndBegin() {
    ...
    if (result) {
        ...
        // No need to call refreshDashboard, SocketIO will trigger it
    }
}
...
function connectWebSocket() {
    ...
    socket.on('connect', () => console.log('✅ Connected to web server via Socket.IO!'));
    
    // Listen for score updates (from device OR web UI)
    socket.on('score_updated', (data) => {
        console.log('EVENT [score_updated]:', data);
    ...
    socket.on('board_state_updated', (data) => {
        console.log('EVENT [board_state_updated]:', data);
        if (scoreboardStates[data.court_id]) {
    ...
    // Listen for general match state changes (begin/finish)
    socket.on('match_state_changed', (data) => {
        console.log(`EVENT [match_state_changed]: ${data.message}`);
        refreshDashboard(); // The simplest way to update the entire UI
    });
    
    socket.on('disconnect', () => console.log('❌ Disconnected from web server.'));
}
...
export default function init() {
    ...
    document.getElementById('attendance-btn').addEventListener('click', async () => {
        if (allPlayers.length === 0) allPlayers = await apiCall('/api/players') || [];
    ...
    document.getElementById('save-attendance-btn').addEventListener('click', async () => {
        // This is a simple action, we can keep the logic here
        const checkboxes = attendanceModal.querySelectorAll('input[type="checkbox"]');
        const active_ids = Array.from(checkboxes).filter(cb => cb.checked).map(cb => cb.dataset.playerId);
        const result = await apiCall('/api/players/attendance', 'POST', { active_player_ids: active_ids });
        if (result) {
            alert(result.message);
    ...
    document.getElementById('save-match-result-btn').addEventListener('click', async () => {
        const matchId = finishModal.querySelector('#finish-match-id-input').value;
